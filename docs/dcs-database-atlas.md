...
0. **Genpilot 对话找数据（默认，第一动作）**：`dcs_biolens_search` 默认在**项目在线容器里走 Genpilot 对话**找数据——Genpilot 助手的 biolens_search（BioLens 组学数据集索引，约 11.7 万条 / 139 来源库，`db.cngb.org/biolens/mcp`）既能命中公开数据集，也能**优先找到容器 /public 里已有的本地数据**；对话返回的只读检索命令由工具在容器内执行验证，给出 /public 真实命中路径/dcs_path。Genpilot 对话无命中时再**宿主直连 BioLens MCP 兜底**公开库（`search_datasets`/`list_dataset_files`：bm25+向量排序、download_url；经 DCS Cloud 集成调用会补 `dcs_path`）。key：`dcs_api_key set biolens <auth_key>`（db.cngb.org/biolens 申请）或 env `DCS_BIOLENS_KEY`。
1. **定组学类别**：按研究问题关键词 → 类别。
...
| --- | --- |
| 查图谱 | `dcs_atlas` |
| 数据检索 | **`dcs_biolens_search`**（Genpilot 对话找数据·容器 /public 优先，第一动作；MCP 兜底 11.7 万条/139 库）→ `dcs_container_ls`/`dcs_data_inspect`（容器 `/public` 验证）→ `dcs_public_search`（公共库元数据）→ `dcs_data_ls/find/info/download` |
