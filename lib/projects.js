...
function normSelfReview(s) {
  ...
  return {
    verdict: ['pass', 'revise', 'rerun'].includes(verdict) ? verdict : verdict,
...
export function buildTrajectory(project) {
  ...
  for (const m of modules) {
    ...
    for (const r of runs) {
      lines.push('\n**v' + r.version + '** · ' + (r.status || 'running') + (r.progress != null && r.progress >= 0 ? ' · ' + r.progress + '%' : '') + (r.finishedAt ? ' · ' + new Date(r.finishedAt).toISOString().slice(0, 19).replace('T', ' ') : ''));
