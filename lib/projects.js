...
export function getProject(projectId, sessionId) {
  const all = loadAll();
  return bestProjectMatch(all, projectId, sessionId);
}

/** 同 projectId 可能有多条历史记录（不同会话 / 旧数据重复），按 updatedAt 取最新，避免改到陈旧记录（doc P2-4）。 */
function bestProjectMatch(all, projectId, sessionId) {
  const matches = all.filter((p) => p.id === projectId && (!sessionId || p.sessionId === sessionId));
  if (!matches.length) return null;
  let best = matches[0];
  for (const p of matches) if ((p.updatedAt || 0) > (best.updatedAt || 0)) best = p;
  return best;
}

function projectIndex(projectId, sessionId) {
  const all = loadAll();
  const best = bestProjectMatch(all, projectId, sessionId);
  return best ? all.indexOf(best) : -1;
}
...
export async function upsertProject(input, sessionId) {
  return withProjectLock(() => {
    ...
    if (input.project_id || input.task_id) {
      const best = bestProjectMatch(all, String(input.project_id || input.task_id), sid);
      idx = best ? all.indexOf(best) : -1;
    } else {
      idx = -1;
    }
    let project;
