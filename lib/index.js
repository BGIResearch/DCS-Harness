...
function explainOfflineError(errText) {
  ...
  if (/unknown shorthand flag|unknown flag/i.test(t)) {
    return '下一步：命令里的 -t/-c 等会被 dcs CLI 当成全局 flag 解析。'
      + '把复杂命令写入脚本文件（先 dcs_terminal_file create），再用 bash /data/work/xxx.sh 投递。';
  }
  return '';
}
...
async function delegateTaskWithGenpilot(cfg, signal, o) {
  ...
  const prompt = [
    ...
    '',
    '要求：',
    '1) 若任务信息不足（缺数据路径/输入文件/目标/参数等关键信息），ready=false 并列出需要补充的具体问题到 questions；',
    '2) 若信息充分或已补充，ready=true，questions 为空数组，给出 commands 完整可执行方案；',
    '3) commands 中 needSaw=true 时第一条是环境初始化命令。',
  ].filter(Boolean).join('\n');
  const llm = await genpilotChat(cfg, signal, prompt, sys, '');
  if (!llm.ok) {
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const build = () => {
        ...
        const a = ['terminal', 'exec', '-c', withSawEnv(cfg, str(args.command))];
  ...
  ctx.tools.register(defineTool({
    ...
    description: '对话式任务委托（Genpilot 对话优先执行）：把任务想法以对话方式提交给 DCS Genpilot，由它理解任务、判断所需环境/镜像/命令并给出可执行方案。**所有投递任务都可以直接以对话方式提交想法给 Genpilot 执行**；若信息不足，Genpilot 会反问（返回 questions），用 answers 补充后再次调用（多轮对话），直到 ready=true。submit=true 时按对话方案直接投递离线任务（needSaw=true 用 SAW 镜像+环境初始化，false 用通用镜像）。',
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        return { ok: r.ok, task_id: r.task_id, task_ids: r.task_ids, output: r.output, truncated: r.truncated, error: r.error };
      } catch (e) {
...
f.close()`;
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_llm',
    description: '调用 DCS 系统 Genpilot LLM（对话模型，默认 deepseek-v4-flash 快速稳定——避开网关 180s 流式超时限制；可显式指定 deepseek-v4-pro 等）做变异解读、文献综合、方案撰写、论文润色、任务失败归因等。在 DCS 在线容器内执行，鉴权由系统自动注入 LLM_API_KEY。json_mode=true 时请求结构化 JSON 输出并解析返回（JSON + 文本摘要双输出），适合失败诊断/参数预检等需要程序化处理的场景。注意：Genpilot chat 是对话模型，与 Genos 预测模型（VCF→RNA 信号，1.2B，需 dcs_api_key 配置）不同。',
    parameters: {
    ...
    async execute(args, exec) {
      ...
      if (!r.ok) return { ok: false, model: r.model, content: r.content, data: null, summary: '', error: r.error };
      ...
      return { ok: true, model: r.model, content: r.content, data, summary: summary || r.content, error: '' };
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    description: '模块对话优先执行顾问（Genpilot 对话驱动）：把模块目标 + 项目上下文（目标/自定义数据/分析计划/依赖产物/可用能力）交给 DCS Genpilot 对话，产出结构化执行方案（plan 概述 + steps 步骤数组[step/action/expect] + risks 风险备选）+ 文本摘要。**每个模块执行前先调用本工具对话定方案**，再按 steps 执行（在线容器/离线任务/WDL/Genpilot 对话）；执行中失败或结果异常也可回灌本工具调整方案。',
    parameters: {
...
（执行阶段·全自动+自批评长循环）用户点「🚀 批准计划并自动执行」后（planStatus=approved）：按 dependsOn 顺序依次完成全部模块（dcs_run_start / dcs_run_update 登记每次运行与产物），**不再设任何审查节点、不暂停、不请示**——可自行决策的问题选合理默认并继续。**模块执行对话优先（强制流程，每模块第一动作=Genpilot chat）**：每个模块开始前，**第一步必须先调 Genpilot 对话（dcs_llm，或投递类用 dcs_task_delegate / 规划类用 dcs_module_consult）**——把模块名称、描述、项目上下文（数据/依赖产物/可用能力）作为 prompt 提交给 Genpilot 对话，让它产出执行方案/步骤/命令；**未经 Genpilot 对话理解模块，不得直接执行命令或投递任务**（对话是每模块的第一动作，不是可选项）。按对话方案执行——执行动作优先用对话给出的命令/流程/镜像；若对话反问（dcs_task_delegate 的 questions），补信息多轮直到 ready；执行中失败、结果异常、参数不确定时**回到对话**（dcs_task_diagnose / dcs_task_delegate / dcs_llm / dcs_module_consult）分析调整，而不是闷头重试。每个模块的 dcs_run_start 的 notes 里记录该模块的对话方案（对话 prompt 摘要 + 产出步骤），dcs_run_update 的 oaa.observation 记录「已按 Genpilot 对话方案执行」的对应关系。**每个模块完成后强制「评估-调整循环」**（对标 Biomni 长 loop，纯执行阶段增强，不动批量审核制）：
...
2. 找数据（容器 /public 第一优先级）：用 dcs_container_ls 列 /public（公共库挂载）——**优先在 BGI Center 片区节点（BGI-时空等）的容器 /public 里找**，公共数据集在 /public/database/CNGBdb/pub/SciRAID/stomics/（STDS 编号）；用 dcs_data_inspect 看 h5ad/csv 结构；**涉 /Files 数据走 dcs-data-manager 规范（dcs table/data find，禁用 Linux find）**；容器 /public 里没有时再用 dcs_public_search 搜公共库元数据、dcs_data_find 查 /Files；避免重复：先 dcs_find_results 看 /work 里 Genpilot 是否已跑过；外部数据下载（dcs_data_download）是最后手段。**若用户提供了自有数据（customData），优先用用户数据，DCS 公共库仅作补充/对照**。
3. 深度研究：用 web_search / 子代理检索文献；需求不明用 ask_user_question 与用户确认，形成研究方案并写入交付文档 question/hypothesis/decomposition。
4. 方案（第一优先级 = 对话式任务委托）：**所有投递/分析任务都可以直接以对话方式把想法提交给 Genpilot 执行**——用 \`dcs_task_delegate\` 把任务想法（要做什么）交给 DCS Genpilot 对话，由它理解任务、判断所需环境/镜像/命令并给出可执行方案；**若 Genpilot 反问补充信息（questions），用 answers 回答后再次调用，多轮对话直到 ready=true**。非投递类模块规划用 \`dcs_module_consult\`（或 dcs_llm）对话确定步骤；**分析计算优先按对话方案交 DCS Genpilot 在线容器交互完成**（dcs_terminal_exec / dcs_terminal_file，写 stepN_*.py 由 Genpilot 环境执行），或复用现有 WDL 流程（dcs_workflow_search / dcs_workflow_info / dcs_workflow_run）；**你只做方案设计与结果把关，不自上而下手写整套分析代码**。执行中问题（投递失败/结果异常/参数不确定）回到对话（dcs_task_diagnose / dcs_task_delegate / dcs_module_consult / dcs_llm）分析解决。仅在 Genpilot 无法覆盖的极少数场景，才用 dcs_audit_script 审计后自行补充。
...
7. 解读与写作：用 dcs_llm 调 DCS Genpilot LLM（对话模型，默认 deepseek-v4-flash 快速稳定、避开网关 180s 超时；复杂任务可显式 deepseek-v4-pro）做变异解读/文献综合/论文写作；dsh 负责把关与整合。如需 Genos 预测（VCF→RNA 信号），用 dcs_api_key 配置后调用 Genos‑VEP / genos‑mutation skill。
