...
function explainOfflineError(errText) {
  ...
  if (/81201/.test(t)) {
    return '下一步：错误码 81201 常见于资源规格缺参——补全 vf=<内存>g,num_proc=<核数>（GPU 加 ,gpu=L4），如 vf=32g,num_proc=8。';
  }
  return '';
}
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '运行时探测一个镜像是否含 python3（及其版本），结果缓存 24h。投一个最小离线任务 `command -v python3 && python3 -V` 等待 settle 后读日志判定；后续 dcs_offline_run / dcs_script_submit 遇 python3 命令会自动查缓存提示（doc P0-4）。',
    parameters: {
      image: { type: 'string', required: true, description: '镜像 registry 路径（public-library/<镜像>:latest）' },
      refresh: { type: 'boolean', description: 'true 时忽略缓存强制重探测' },
      resource: { type: 'string', description: '探测任务资源，默认 vf=4g,num_proc=1' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
    ...
    async execute(args, exec) {
      ...
      if (!image) return { ok: false, image: '', has_python3: null, python_version: '', cached: false, task_id: '', output: '缺少 image', error: 'missing image' };
      if (!args.refresh) {
        const hit = cachedImageProbe(image);
        if (hit) {
          return { ok: true, image, has_python3: hit.hasPy3, python_version: hit.pyVersion || '', cached: true, task_id: '', output: '命中 24h 缓存' + (hit.error ? '（上次探测失败：' + hit.error + '）' : ''), error: hit.error || '' };
        }
      }
      const resSpec = args.resource || 'vf=4g,num_proc=1';
      const res = checkDcsResource(resSpec);
      if (res.error) return { ok: false, image, has_python3: null, python_version: '', cached: false, task_id: '', output: res.error, error: res.error };
      // 探测命令：有 python3 打 PROBE_PY3:<版本>，无则 PROBE_PY3:NONE
      ...
      try {
        ...
        if (!r.ok) {
          ...
          return { ok: false, image, has_python3: null, python_version: '', cached: false, task_id: r.task_id || '', output: r.output || '', error: r.error || '投递失败' };
        }
        ...
        // 兜底：任务完成但未捕获 marker（缺 marker 多为环境异常）→ 视为无 python3 结论，随后可 refresh 重探
        if (hasPy3 === null) hasPy3 = false;
        imageProbeCache.set(image, { hasPy3, pyVersion, at: Date.now(), error: '' });
        return {
          ok: true, image, has_python3: hasPy3, python_version: pyVersion, cached: false, task_id: taskId,
          output: 'python3 探测结果：' + (hasPy3 ? '有（' + pyVersion + '）' : '无') + '（任务状态 ' + state + '）',
          error: '',
        };
      } catch (e) {
        return { ok: false, image, has_python3: null, python_version: '', cached: false, task_id: '', output: String(e && e.message || e), error: String(e && e.message || e) };
      }
    },
  }));
