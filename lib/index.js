...
async function submitDcsTask(cfg, exec, opts) {
  ...
  // 资源格式本地校验：归一化后仍不合法直接拒绝投递，不把非法格式交给 CLI/Pod 报错
  let resource = '';
  if (str(opts.resource)) {
    const rc = checkDcsResource(opts.resource);
    if (rc.error) return { ok: false, channel: 'local', task_id: '', task_ids: [], output: rc.error, error: rc.error, truncated: false };
    resource = rc.value;
  }
  ...
  if (kind === 's') {
    try {
      ...
      if (str(opts.command)) podArgs.push('-i', str(opts.command));
      if (str(opts.batch_file)) podArgs.push('-p', str(opts.batch_file));
      if (resource) podArgs.push('-l', resource);
      ...
      if (!isChannelUnavailable(podErr) && !isFileVisibilityError(podErr)) {
        // 业务/参数/资源/镜像校验失败：直接报回，不掩盖真实原因、不降级；附已知错误的「下一步」提示
        const hint = explainOfflineError(podErr);
        const errFull = hint ? podErr + '\n' + hint : podErr;
        const text = (termStdout(r) || errFull || '').slice(0, OUTPUT_CAP);
        return { ok: false, channel: 'pod', task_id: '', task_ids: [], output: text, error: errFull, truncated: text.length > OUTPUT_CAP };
      }
  ...
  try {
    ...
    if (kind === 's') {
      a = ['analysis', 'run'];
      if (str(opts.command)) a.push('-i', str(opts.command));
      if (str(opts.batch_file)) a.push('-p', str(opts.batch_file));
      if (resource) a.push('-l', resource);
    ...
    if (!r.ok) {
      // 宿主通道失败：带 --debug 重跑一次抓底层 api_msg，并附已知错误的「下一步」提示
      const errText = await diagnoseOfflineFailure(cfg, a, sig, r.error || r.message || '投递失败');
      return { ok: false, channel: 'host', task_id: '', task_ids: [], output: errText, error: errText, truncated: false };
    }
    const ids = parseTaskIdsFromResult(r);
...
5. 执行（监督与登记）：指导 Genpilot 用 dcs_terminal_exec 在线跑分析；**长任务/并行 → dcs_offline_run（资源 "4c 16g" 或 "vf=32g,num_proc=8" 均可，插件本地校验并自动转换；会返回 task_id）或 dcs_parallel_run（返回 task_ids）**。**投递默认由插件走 Genpilot Pod 内 \`dcs task run\`（-m 自动挂载输入文件，WDL 引擎才能看到外部导入实体如 VIRE-* 的文件），Pod 通道不可用时自动降级宿主 CLI**；离线任务输入涉及 /Files 或外部导入实体时，显式给 mount/input 路径让插件自动推导 -m 挂载。离线任务法则：**镜像须为公共库注册路径 public-library/<镜像名>:latest（Docker Hub 短名如 ubuntu:24.04 会报 image_url不存在，投递前用 dcs_public_search 检索 resType=img 确认）；离线容器工作目录 = /data/work（非在线容器的 /work/{user}），/Files、share-data 只读；挂载数据在容器内需补全 /data/input/ 前缀访问；复杂命令先写成脚本文件再 bash 执行，避免 -t/-c 被 CLI 当全局 flag**。**任务归属：带特定目标数据的任务必须在数据所在的已有项目投递（另开新项目引擎看不到输入）；投递前用 dcs_status 确认当前项目；DCS 项目间数据不共享，跨项目数据先用 dcs data copy --target-project <数据所在项目> 拷贝**。每个模块的每次运行：dcs_run_start 开启 → 监督执行 → dcs_run_update 更新状态/进度/文件清单（code/input/output）/notes/**oaa**，**并把离线任务返回的 task_id / task_ids 填入 dcs_task_ids 字段**（供「项目管理」窗口拉取实时费用）。**OAA 是硬性要求**：每个模块完成后必须写 observation/assessment/action 三段；失败或数据异常先 dcs_self_review 定位原因，再决定重跑 v2（dcs_run_start 自动 v2）还是调整后续模块。
