...
`;
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'DCS Harness v2：在项目里创建/更新一个分析模块（任务分解的最小可执行单元，如「数据预处理」「差异表达分析」「可视化」）。模块是「项目管理」窗口的核心条目。支持 human-in-the-loop：用 isGate 把某模块标记为人工把关闸门（check 节点，执行到这里必须等人批准），用 dependsOn 声明依赖，用 review 记录审阅状态。',
    parameters: {
      ...
      isGate: { type: 'boolean', description: '是否为人工把关闸门节点（true=check 节点，执行到此必须等人在「项目管理」窗口批准后才能继续）' },
      review: { type: 'string', enum: ['none', 'gate', 'reviewing', 'approved', 'rejected'], description: '审阅状态：none 无闸门 / gate 待审批 / approved 已批准 / rejected 已否决（AI 分解时设 gate，人审批后变 approved）' },
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // ---- Human-in-the-loop：节点审批（设想/批准/纠正/否决） ----
...
C2. **human-in-the-loop 节点把关（贯穿执行）**：把「需要人工确认的关键步骤」用 dcs_module_update 的 isGate=true 标记为闸门节点（check 节点），并用 dependsOn 声明步骤依赖。AI 分解模块时，在每个大步骤（差异分析、变体解读、跨库结论汇合等）后设一个 check 闸门；执行到 check 节点时停下，等人在「项目管理」窗口对该节点点「批准/纠正/否决」。人被否决的节点用 review=rejected 回退重设计，被纠正的节点回到 review=gate 重新待审。**「AI 分解、人工支配」**：AI 全局自治地计划与执行，但每个 check 闸门必须经人批准才放行。
