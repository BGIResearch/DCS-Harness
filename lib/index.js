...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'BioLens 数据库检索（宿主直连 biolens-search MCP 服务 https://db.cngb.org/biolens/mcp —— OmicSeek 数据检索，DCS Genpilot 对话里自动启用的 biolens-search 即同一服务）。用中文或英文自然语言快速检索生物组学数据集（覆盖 CNGBdb 数据仓库 / CROST / NCBI GEO / CZ CELLxGENE / SpatialGWAS / MOSTA 时空图谱等），bm25+向量混合打分排序；经 DCS Cloud 集成（Genpilot 容器对话）调用时 list_dataset_files 会补 dcs_path 容器内路径，实现「优先命中容器 /public 已有数据」；宿主直连给出 download_url/access_url 与可用性。execute=true（默认）对命中前 files 条自动取文件清单。需先配置 key：dcs_api_key set biolens <auth_key>（db.cngb.org/biolens 申请，与 Genpilot 同 key）。检索通常 1-5s，比 find 全盘扫快得多。只检索不下载不分析。**找数据流程首选本工具**。',
    parameters: {
      query: { type: 'string', required: true, description: '要找的数据（中文/英文自然语言均可，尽量写全：物种+组学+时期/组织），如 "小鼠脑时空图谱 E16.5 全脑 Stereo-seq"、"人类 心肌 单细胞"、"BRCA1 人源蛋白结构"' },
      limit: { type: 'integer', description: '返回数据集条数上限，默认 10，最大 100' },
      files: { type: 'integer', description: '对命中前 N 条数据集取文件清单，默认 3（0=只看数据集摘要）' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
        properties: {
          ok: { type: 'boolean', required: true },
          meta: { type: 'string', description: '检索计数（命中总数/返回条数）' },
          datasets: { type: 'string', required: true, description: '按分数排序的数据集命中列表（record_id/source/标题/可用性/分数）' },
          files: { type: 'string', description: '命中数据集的关联文件清单（file_name/download_url/dcs_path，files>0 时）' },
          hint: { type: 'string', description: '使用提示（容器路径验证 / 下一步）' },
          error: { type: 'string' },
        },
      },
      render(args, v) {
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        if (!key) {
          return { ok: false, meta: '', datasets: '', files: '', hint: '', error: '未配置 biolens MCP key：请用 dcs_api_key set biolens <auth_key> 配置（从 db.cngb.org/biolens 申请，与 Genpilot biolens-search 同一 key）' };
        }
        const q = String(args.query || '').trim();
        if (!q) return { ok: false, meta: '', datasets: '', files: '', hint: '', error: 'query 必填' };
        const limit = Math.min(100, Math.max(1, Math.floor(args.limit || 10)));
        const filesN = Math.max(0, Math.floor(args.files === undefined ? 3 : args.files));
        const s = await biolensMcpCall(cfg, exec && exec.signal, 'tools/call', { name: 'search_datasets', arguments: { query: q, limit: limit } });
        if (!s.ok) return { ok: false, meta: '', datasets: '', files: '', hint: '', error: 'biolens MCP 调用失败：' + s.error };
        ...
        for (const it of toExpand) {
          const f = await biolensMcpCall(cfg, exec && exec.signal, 'tools/call', { name: 'list_dataset_files', arguments: { dataset_record_id: it.record_id, limit: 10 } });
        ...
        const hint = withPath
          ? '以上命中已含 dcs_path 容器内路径，可用 dcs_data_inspect / dcs_container_ls 验证结构后直接使用（容器 /public 已有数据优先）。'
          : '宿主直连暂未返回 dcs_path：如需容器内路径，可在 Genpilot 对话（项目容器）里用同 key 走 biolens-search（DCS Cloud 集成会补 dcs_path），或先用 dcs_container_ls 检查 /public/database/CNGBdb/pub/SciRAID 等是否已镜像该库，再按 download_url 取数。';
        return { ok: true, meta: '命中总数=' + (total != null ? total : items.length) + '（返回 ' + items.length + ' 条）', datasets: lines.join('\n'), files: filesText, hint: hint, error: '' };
      } catch (e) {
        return { ok: false, meta: '', datasets: '', files: '', hint: '', error: String((e && e.message) || e) };
      }
    },
  }));
...
A. **学术检索**：先了解整个研究背景。用 web_search / 子代理检索该领域的关键文献、物种/疾病背景、已知结果与常用分析范式；**找数据第一动作 = dcs_biolens_search（BioLens MCP 直连检索：中英文自然语言命中 OmicSeek 数据集库，1-5s 出结果；Genpilot/DCS 集成调用同源 MCP 会给容器 dcs_path，容器 /public 已有数据优先）**，再用 dcs_atlas 查「数据库全图谱」（片区公共库 + 官方组学工具库 + 关键词映射 + 容器公共数据集）+ dcs_container_ls 列 /public 验证命中、dcs_public_search 搜公共库元数据、dcs_find_results 看 /work 已有分析——**摸清"这个领域有什么可用的数据资源、有哪些成熟流程可复用"**。
...
2. 找数据（**第一动作 = dcs_biolens_search**：BioLens MCP（OmicSeek）中英文自然语言检索，宿主直连 1-5s 出排序结果；经 Genpilot/DCS 集成调用同源 MCP 会给 dcs_path 容器路径——**优先命中容器 /public 已有公共数据**，比手动逐目录 find 快；容器 /public 仍是第一优先级）：命中后可用 dcs_container_ls 列 /public（公共库挂载）验证路径、dcs_data_inspect 看 h5ad/csv 结构——**优先在 BGI Center 片区节点（BGI-时空等）的容器 /public 里找**，公共数据集在 /public/database/CNGBdb/pub/SciRAID/stomics/（STDS 编号）；**涉 /Files 数据走 dcs-data-manager 规范（dcs table/data find，禁用 Linux find）**；容器 /public 里没有时再用 dcs_public_search 搜公共库元数据、dcs_data_find 查 /Files；避免重复：先 dcs_find_results 看 /work 里 Genpilot 是否已跑过；外部数据下载（dcs_data_download）是最后手段。**若用户提供了自有数据（customData），优先用用户数据，DCS 公共库仅作补充/对照**。
