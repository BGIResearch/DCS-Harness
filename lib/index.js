...
/** 预投递提示：命令需要 python3 但镜像疑似 SAW（PATH 常无 python3）时给一句提醒（doc P0-4）。 */
function imagePyHint(image, command) {
  if (!imageLikelySaw(image)) return '';
  if (!/\bpython3?\b/.test(String(command || ''))) return '';
  return '⚠️ 该镜像疑似 SAW（PATH 常无 python3），而命令需要 python3：必要时在脚本内先 source env.sh，或换成含 python3 的 CellBin2 镜像（fanjinghong_...）。';
}
