...
import { readFileSync, existsSync, mkdirSync, copyFileSync, statSync, createReadStream } from 'node:fs';
...
/** 项目文件 tab 的本地缓存目录（容器文件下载到本地，供预览/下载/离线复用）。 */
function localFileDir() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dcs-file-cache');
}

/** 把任意容器文件下载到本地缓存（大文件可配超时；路径 sha1 去重，已存在复用）。 */
async function fetchContainerFile(cfg, remotePath, timeoutMs) {
  try {
    mkdirSync(localFileDir(), { recursive: true });
    const local = join(localFileDir(), 'f-' + createHash('sha1').update(String(remotePath)).digest('hex').slice(0, 20) + (extname(remotePath) || '.bin'));
    if (existsSync(local)) return local;
    const r = await runDcs(cfg, ['terminal', 'download', '-p', remotePath, '-t', local], { timeoutMs: timeoutMs || 600000 });
    if (!r.ok) return null;
...
/** 容器路径安全校验：仅允许在线容器可浏览的绝对根（/work /public /home），拒绝 .. 与裸 /。 */
function safeContainerPath(p) {
  const s = String(p || '').trim();
  if (!s.startsWith('/')) return null;
  if (s.split('/').includes('..')) return null;
  if (!/^\/(work|public|home)(\/|$)/.test(s)) return null;
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '找数据检索（**默认片区 = 时空片区 BGI-时空；默认优先走项目容器内 Genpilot 对话，容器 /public 已有数据优先**）：把找数据任务以对话方式提交给 Genpilot——平台侧 Genpilot 助手（web/容器对话）的 biolens_search 工具（BioLens 组学数据集索引，约 11.7 万条/139 来源库）可命中公开数据并给出容器 dcs_path；对话返回的只读检索命令由本工具在在线容器内执行验证，**优先返回容器 /public 真实存在的命中路径**（如 /public/database/CNGBdb/pub/SciRAID/stomics/STDS*）。**宿主直连 BioLens MCP（db.cngb.org/biolens/mcp，search_datasets/list_dataset_files）始终作为补充通道**（mcp=true 默认开：无论 Genpilot 是否命中，都并行给出公开库记录与文件供对照；对话不可用时 MCP 独立兜底）。注意：容器 /public 数据只在 BGI Center 片区（BGI-时空等）挂载——当前片区非时空时，请先 dcs region switch BGI-时空（或项目管理窗口切换节点）再跑，否则容器验证会拿不到 /public。只检索不下载不分析。**找数据第一动作 = 本工具**。耗时预期 30-90s（Genpilot 推理固有耗时），请耐心等待，勿因超时重复调用。',
    parameters: {
      ...
      mcp: { type: 'boolean', description: 'true=宿主直连 BioLens MCP 始终作为补充通道，并行给出公开库记录与文件（默认 true；需已配置 biolens key：dcs_api_key set biolens <auth_key>）' },
      limit: { type: 'integer', description: 'MCP 补充通道返回条数上限，默认 10，最大 100' },
      files: { type: 'integer', description: 'MCP 补充通道对命中前 N 条数据集取文件清单，默认 3（0=只看摘要）' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
        properties: {
          ...
          channel: { type: 'string', description: '实际命中通道：genpilot+容器+mcp补充 / genpilot+容器 / genpilot / mcp（Genpilot 不可用时的 MCP 兜底）' },
          datasets: { type: 'string', description: 'Genpilot 对话结果与命中候选（容器真实路径优先）' },
          files: { type: 'string', description: '公开库补充记录的文件清单（MCP 补充通道，含 download_url/dcs_path）' },
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        // B) 宿主直连 BioLens MCP：**补充通道**——只要 mcp!=false 就并行查公开库记录/文件，
        //    与 Genpilot/容器命中并列展示（容器命中仍优先，MCP 负责补公开库/下载对照）
        let mcpOk = false, mcpFiles = '', mcpErr = '', mcpHint = '';
        if (args.mcp !== false) {
          const m = await mcpFallback(cfg, sig, q, args);
          if (m.ok) { mcpOk = true; mcpFiles = m.files; mcpHint = m.hint; }
          else mcpErr = m.error || '';
        }
        const datasetsTxt = '【Genpilot 对话结果】' + (d.summary || '') + (d.notes ? '\n【Genpilot notes】' + d.notes : '');
        const channelBits = ['genpilot'];
        if (evidence) channelBits.push('容器');
        if (mcpOk && mcpFiles) channelBits.push('mcp补充');
        let hint;
        if (hasContainerHit && mcpOk) hint = '容器 /public 命中优先（可直接 dcs_data_inspect/dcs_container_ls 验证使用）；MCP 补充公开库记录与文件，可交叉对照。';
        else if (hasContainerHit) hint = '已优先给出容器 /public 真实命中路径，可用 dcs_data_inspect / dcs_container_ls 验证结构后直接使用。';
        else if (mcpOk) hint = 'Genpilot 未给出容器命中，MCP 已补充公开库记录/文件（宿主直连无 dcs_path；容器 dcs_path 需走 Genpilot/DCS 集成调用）。';
        else hint = 'Genpilot 未给出容器命中，且 MCP 补充失败：' + (mcpErr || '请先 dcs_api_key set biolens <auth_key> 配置 key 后重试');
        return {
          ok: true, ready: true,
          channel: channelBits.join('+'),
          datasets: datasetsTxt, files: mcpOk ? mcpFiles : '', evidence, questions: [],
          hint, error: '',
        };
      } catch (e) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // ===================== 项目文件 tab（浏览 / 预览 / 下载） =====================
        // 容器目录 python 列目录 → JSON（dir/name/type/size/mtime）
        const listDirPy = (dir) => `import os,json,sys
p=${JSON.stringify(dir)}
if not os.path.isdir(p):
    print(json.dumps({'error':'not_a_directory','path':p})); sys.exit(0)
out=[]
for n in sorted(os.listdir(p)):
    fp=os.path.join(p,n)
    try: st=os.lstat(fp)
    except Exception: continue
    try: isdir=os.path.isdir(fp) and not os.path.islink(fp)
    except Exception: isdir=False
    out.append({'name':n,'type':'dir' if isdir else 'file','size':st.st_size,'mtime':int(st.st_mtime)})
print(json.dumps({'dir':p,'entries':out[:1000]}))`;
        const runList = async (dir) => {
          const r = await termExec(loadCfg(), ['terminal', 'exec', '-c', "python3 - <<'PYEOF'\n" + listDirPy(dir) + "\nPYEOF", '--timeout', '60'], { timeoutMs: 120000 });
          if (!r.ok) throw new Error('容器执行失败：' + (r.error || r.stderr || ''));
          const out = termStdout(r) || '';
          for (const ln of out.split('\n').reverse()) {
            ...
            try {
              ...
              if (j && (Array.isArray(j.entries) || j.error)) return j;
            } catch { /* 非目标行 */ }
          }
          return { error: '无法解析容器输出', raw: out.slice(0, 500) };
        };
        // GET /v2/files/status → 当前项目/用户/容器 home（默认 /work/{user}）
        if (req.method === 'GET' && path === '/v2/files/status') {
          try {
            const cfg = loadCfg();
            const st = await statusSnapshot(cfg);
            const home = st.username ? '/work/' + st.username : '/work';
            json(200, { ok: true, home, loggedIn: st.loggedIn, username: st.username, region: st.region, project: st.project, projectName: st.projectName, roots: ['/work', '/public'] });
          } catch (e) { json(500, { ok: false, error: String(e && e.message || e) }); }
          return;
        }
        // GET /v2/files/list?path=/work/... → { ok, dir, entries:[{name,type,size,mtime}] }
        if (req.method === 'GET' && path === '/v2/files/list') {
          const dir = safeContainerPath(url.searchParams.get('path') || '');
          if (!dir) { json(200, { ok: false, error: '非法路径（仅允许 /work /public /home 下的容器绝对路径）' }); return; }
          try {
            const j = await runList(dir);
            if (j && j.error) { json(200, { ok: false, error: j.error === 'not_a_directory' ? '目录不存在: ' + dir : ('列目录失败：' + j.error) }); return; }
            json(200, { ok: true, dir: (j && j.dir) || dir, entries: (j && j.entries) || [] });
          } catch (e) { json(200, { ok: false, error: String(e && e.message || e) }); }
          return;
        }
        // GET /v2/files/read?path=... → 小文件在线预览（文本内容 / 图片 base64 / 二进制标记）
        if (req.method === 'GET' && path === '/v2/files/read') {
          const p = safeContainerPath(url.searchParams.get('path') || '');
          if (!p) { json(200, { ok: false, error: '非法路径' }); return; }
          try {
            const cfg = loadCfg();
            const local = await fetchContainerFile(cfg, p, 180000);
            if (!local || !existsSync(local)) { json(200, { ok: false, error: '文件不存在或下载失败：' + p }); return; }
            const st = statSync(local);
            if (!st.isFile()) { json(200, { ok: false, error: '不是常规文件' }); return; }
            const ext = extname(p).toLowerCase();
            const isImg = ['.png', '.jpg', '.jpeg', '.gif', '.webp'].includes(ext);
            const isSvg = ext === '.svg';
            const TEXT_RE = /\.(txt|csv|tsv|tab|md|markdown|log|out|err|json|ya?ml|py|sh|r|rmd|ipynb|js|ts|jsx|tsx|c|h|cpp|java|go|html?|xml|vcf|bed|gtf|gff|fa|fasta|fq|fastq|pl|sql|toml|ini|cfg|env|rds)$/i;
            if ((isImg || isSvg) && st.size <= 6 * 1024 * 1024) {
              const b64 = readFileSync(local).toString('base64');
              const mime = isSvg ? 'image/svg+xml' : ({ '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp' })[ext];
              json(200, { ok: true, type: 'image', name: basename(p), path: p, size: st.size, content: 'data:' + mime + ';base64,' + b64 });
              return;
            }
            if (TEXT_RE.test(p) && st.size <= 4 * 1024 * 1024) {
              const txt = readFileSync(local).toString('utf8');
              const content = txt.length > 400000 ? txt.slice(0, 400000) + '\n\n…（预览截断，如需全文请下载）' : txt;
              json(200, { ok: true, type: 'text', name: basename(p), path: p, size: st.size, content });
              return;
            }
            if (ext === '.pdf' && st.size <= 20 * 1024 * 1024) {
              json(200, { ok: true, type: 'pdf', name: basename(p), path: p, size: st.size, content: '' });
              return;
            }
            json(200, { ok: true, type: 'binary', name: basename(p), path: p, size: st.size, tooLarge: st.size > 4 * 1024 * 1024, content: '' });
          } catch (e) { json(200, { ok: false, error: String(e && e.message || e) }); }
          return;
        }
        // GET /v2/files/download?path=... → 流式下载容器文件（任意大小，attachment）
        if (req.method === 'GET' && path === '/v2/files/download') {
          const p = safeContainerPath(url.searchParams.get('path') || '');
          if (!p) { json(400, { ok: false, error: '非法路径' }); return; }
          try {
            const cfg = loadCfg();
            const local = await fetchContainerFile(cfg, p, 1200000);
            if (!local || !existsSync(local)) { json(200, { ok: false, error: '文件不存在或下载失败：' + p }); return; }
            const st = statSync(local);
            if (!st.isFile()) { json(200, { ok: false, error: '不是常规文件' }); return; }
            const fname = basename(p) || 'download';
            res.writeHead(200, {
            ...
            rs.on('error', () => { try { res.destroy(); } catch { /* 客户端断开 */ } });
            req.on('close', () => { if (!res.writableEnded) { try { rs.destroy(); } catch { /* 忽略 */ } } });
            rs.pipe(res);
          } catch (e) { json(500, { ok: false, error: String(e && e.message || e) }); }
          return;
        }
...
A. **学术检索**：先了解整个研究背景。用 web_search / 子代理检索该领域的关键文献、物种/疾病背景、已知结果与常用分析范式；**找数据第一动作 = dcs_biolens_search（默认片区 = 时空片区 BGI-时空；默认走容器内 Genpilot 对话找数据：Genpilot 助手的 biolens_search 既能命中公开数据集，也能优先给出容器 /public 已有数据的 dcs_path；宿主 BioLens MCP 始终作为补充通道并行给出公开库记录与文件）**，再用 dcs_atlas 查「数据库全图谱」（片区公共库 + 官方组学工具库 + 关键词映射 + 容器公共数据集）+ dcs_container_ls 列 /public 验证命中、dcs_public_search 搜公共库元数据、dcs_find_results 看 /work 已有分析——**摸清"这个领域有什么可用的数据资源、有哪些成熟流程可复用"**。
...
2. 找数据（**第一动作 = dcs_biolens_search，默认走容器内 Genpilot 对话；默认片区 = 时空片区 BGI-时空**——容器 /public 数据在 BGI Center 片区（BGI-时空等）挂载，当前片区非时空时先 dcs region switch BGI-时空 或项目管理切换节点再检索）：在项目在线容器里让 Genpilot 找数据——它的 biolens_search 既能命中约 11.7 万条公开组学数据集，也能**优先找到容器 /public 里已有的本地数据**（对话返回只读命令、由工具在容器执行验证，给出 /public 真实路径/dcs_path）；**宿主 BioLens MCP 始终作为补充通道**（mcp=true 默认：并行给出公开库记录与文件供对照）；**容器 /public 仍是第一优先级**）：命中后可用 dcs_container_ls 列 /public（公共库挂载）验证路径、dcs_data_inspect 看 h5ad/csv 结构——**优先在 BGI Center 片区节点（BGI-时空等）的容器 /public 里找**，公共数据集在 /public/database/CNGBdb/pub/SciRAID/stomics/（STDS 编号）；**涉 /Files 数据走 dcs-data-manager 规范（dcs table/data find，禁用 Linux find）**；容器 /public 里没有时再用 dcs_public_search 搜公共库元数据、dcs_data_find 查 /Files；避免重复：先 dcs_find_results 看 /work 里 Genpilot 是否已跑过；外部数据下载（dcs_data_download）是最后手段。**若用户提供了自有数据（customData），优先用用户数据，DCS 公共库仅作补充/对照**。
