...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_configure',
    description: '配置 dcs CLI 的本地参数（cliPath、autoInstall）与技能 API key（api_keys，如 genos_vep → HG38_VCF_PREDICT_API_KEY）。Genpilot LLM 与 Genos 为 DCS 系统自带、模型自动选择、无需额外 API key。PAT 登录请用 dcs_login；单个 API key 管理用 dcs_api_key。',
    parameters: {
    ...
    output: {
      ...
      render(args, v) { return [{ type: 'text', text: v.ok ? ('✅ dcs 配置已保存：cliPath=' + v.cliPath + '，autoInstall=' + v.autoInstall + '（平台 ' + v.platform + '）。技能 API key：' + (v.apiKeys && v.apiKeys.length ? v.apiKeys.join(', ') : '（无）')) : ('❌ 配置失败: ' + (v.error || '')) }]; },
    },
    async execute(args) {
      const cfg = loadCfg();
      const next = { ...cfg };
...
`;
  ...
  ctx.systemPrompt.section({
    name: 'dcs-cloud',
    order: 160,
    text: () => `【DCS 云研究（DCS Harness v2·Genpilot 默认）】本会话已接入 DCS Cloud（通过 dcs CLI，https://github.com/BGIResearch/dcs_cli）。**默认引擎 = DCS Genpilot**：你是「使用科学家」导师，只负责分解科学问题、制定方案、指导与监督；**一切分析计算默认交由 DCS Genpilot 智能交互执行**（在线容器交互 / 离线并行 / Genpilot LLM），**不自上而下手写整套分析代码**。**每次使用均为独立的 Genpilot 项目**（一次使用=一个 DCS 项目）。遵循以下流程（**用户输入问题后，先规划、再执行，绝不直接开跑**）：
