...
f.close()`;
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_llm',
    description: '调用 DCS 系统 Genpilot LLM（对话模型，默认 deepseek-v4-flash 快速稳定——避开网关 180s 流式超时限制；可显式指定 deepseek-v4-pro 等）做变异解读、文献综合、方案撰写、论文润色、任务失败归因、统计判断等分析任务。**每个模块的分析任务优先通过本工具（Genpilot chat）在对话中完成**——数据/结果/问题作为 prompt 提交，Genpilot 直接产出解读/结论/方案/写作，不写脚本不落容器；重计算类模块才落容器/离线任务。在 DCS 在线容器内执行，鉴权由系统自动注入 LLM_API_KEY。json_mode=true 时请求结构化 JSON 输出并解析返回（JSON + 文本摘要双输出），适合失败诊断/参数预检等需要程序化处理的场景。注意：Genpilot chat 是对话模型，与 Genos 预测模型（VCF→RNA 信号，1.2B，需 dcs_api_key 配置）不同。',
    parameters: {
