...
async function submitDcsTask(cfg, exec, opts) {
  ...
  try {
    ...
    if (!r.ok) return { ok: false, channel: 'host', task_id: '', task_ids: [], output: r.error || r.message || '', error: r.error || r.message || '', truncated: false };
    const ids = parseTaskIdsFromResult(r);
...
5. 执行（监督与登记）：指导 Genpilot 用 dcs_terminal_exec 在线跑分析；**长任务/并行 → dcs_offline_run（资源 "4c 16g" 或 "vf=32g,num_proc=8" 均可，插件自动转换；会返回 task_id）或 dcs_parallel_run（返回 task_ids）**；标准镜像 ubuntu:24.04-python3.12。**投递默认由插件走 Genpilot Pod 内 \`dcs task run\`（-m 自动挂载输入文件，WDL 引擎才能看到外部导入实体如 VIRE-* 的文件），Pod 通道不可用时自动降级宿主 CLI**；离线任务输入涉及 /Files 或外部导入实体时，显式给 mount/input 路径让插件自动推导 -m 挂载。每个模块的每次运行：dcs_run_start 开启 → 监督执行 → dcs_run_update 更新状态/进度/文件清单（code/input/output）/notes/**oaa**，**并把离线任务返回的 task_id / task_ids 填入 dcs_task_ids 字段**（供「项目管理」窗口拉取实时费用）。**OAA 是硬性要求**：每个模块完成后必须写 observation/assessment/action 三段；失败或数据异常先 dcs_self_review 定位原因，再决定重跑 v2（dcs_run_start 自动 v2）还是调整后续模块。
