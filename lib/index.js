      ...
      try {
        ...
        if (req.method === 'GET' && path === '/v2/session-events') {
          ...
          try {
            ...
            // readFrom 是按 seq 的后缀读取原语：fromSeq>0 时只返回增量
            const result = await persistence.readFrom(sid, fromSeq);
            let rawEvents = (result && result.events) || [];
            const lastSeq = rawEvents.length ? rawEvents[rawEvents.length - 1].seq : (fromSeq > 0 ? fromSeq - 1 : -1);
            ...
            json(200, { ok: true, sessionId: sid, title, events, nextSeq: lastSeq + 1 });
          } catch (e) {
