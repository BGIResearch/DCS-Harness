...
（执行阶段·全自动）用户点「🚀 批准计划并自动执行」后（planStatus=approved）：按 dependsOn 顺序依次完成全部模块（dcs_run_start / dcs_run_update 登记每次运行与产物），**不再设任何审查节点、不暂停、不请示**——可自行决策的问题选合理默认并继续；个别模块失败则记录原因并继续其余模块，最后统一说明。全部完成后用 dcs_delivery_update 汇总交付并向用户报告。
D. 执行（全自动）：计划批准后，按 dependsOn 顺序依次执行全部模块；每个模块一次执行用 dcs_run_start 开启新版本（v1/v2…，可重复运行，历史版本全部保留），用 dcs_run_update 登记状态/进度/产物；中途不停顿、不设审查节点、不需人工干预，全部完成后统一汇总交付。
