...
/** 失败诊断：带 --debug 重跑同一命令（仅抓底层 api_msg，不改语义），拼出完整错误。 */
...
async function skillCatalog(cfg, exec) {
...
print(json.dumps(out))`;
  ...
  try {
    const list = JSON.parse(termStdout(r));
    return { ok: true, list };
  } catch (e) {
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        const cmdArgs = build();
        const r = await runDcs(cfg, cmdArgs, { signal: sig, timeoutMs: 180000 });
        if (!r.ok) {
          const errText = await diagnoseOfflineFailure(cfg, cmdArgs, sig, r.error || r.message || '投递失败');
          return { ok: false, task_id: '', task_ids: [], error: errText, output: errText, truncated: false };
        }
...
`;
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const allTaskIds = [];
      let submitted = 0;
      let diagnosed = false; // 同样的错误只需 --debug 诊断一次
      for (let i = 0; i < count; i++) {
        const cmd = tpl.indexOf('{i}') !== -1 ? tpl.split('{i}').join(String(i)) : tpl;
        const a = ['analysis', 'run', '-i', cmd, '-l', res.value, '--image', str(args.image)];
        ...
        try {
          ...
          let errText = r.ok ? '' : (r.error || r.message || '投递失败');
          if (!r.ok && !diagnosed) { diagnosed = true; errText = await diagnoseOfflineFailure(cfg, a, sig, errText); }
          results.push('分片 ' + i + ': ' + (r.ok ? ('✅ 已投递 ' + (tid || '') + ' ' + (r.message || '')) : ('❌ ' + errText)) + '  [' + cmd.slice(0, 60) + ']');
        } catch (e) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '列出/检索 DCS Genpilot 平台的「原生技能库」（容器 /public/skills：dcs-skills / builtin_skills / OmicsClaw / LabClaw / bioSkills / claude-scientific-skills / ClawBio，共 973 条）。支持按类别、关键词、仅原生过滤。用于在发起分析前发现可复用的 DCS 原生 skill。',
    parameters: {
      category: { type: 'string', description: '顶级类别过滤：dcs-skills / builtin_skills / OmicsClaw / LabClaw / bioSkills / claude-scientific-skills / ClawBio' },
    ...
    output: {
      ...
      render(args, v) { return [{ type: 'text', text: (v.ok ? '📚 DCS 原生技能（' + v.count + ' 条）：\n' : '❌ ' + (v.error || '') + '\n') + v.output }]; },
    },
    timeoutMs: 120000,
    async execute(args, exec) {
      const cfg = loadCfg();
      ...
      const catalog = await skillCatalog(cfg, exec);
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_skill_read',
    description: '读取一个 DCS Genpilot 原生技能的完整内容（SKILL.md，其次 README.md/AGENTS.md）。name 传技能路径或名称，如 "dcs-skills/genos-mutation"、"OmicsClaw/bulkrna-bulkrna-de"；只传叶子名（如 "genos-mutation"）会自动在技能库中定位。',
    parameters: {
      name: { type: 'string', required: true, description: '技能路径或名称（相对 /public/skills），如 dcs-skills/genos-mutation；只传叶子名会按名称定位' },
    },
    output: {
    ...
    async execute(args, exec) {
      ...
      if (!name.includes('/')) {
        ...
        if (catalog.ok) {
          ...
          if (hit) rel = (hit.path || '').replace(SKILLS_BASE + '/', '').replace(/\/SKILL\.md$/i, '') || hit.name;
          else rel = name;
        }
      }
...
0.5. 优先复用 DCS 原生 skill / 专家：发起分析前，先 dcs_skills_list（按 category/keyword/native 检索）→ dcs_skill_read 读 SKILL.md，复用可用的 DCS 原生 skill；需要某方向专家（单细胞/空间/WGS-WES/临床肝癌/感染等）用 dcs_experts_list → dcs_expert_read 读取 AGENTS.md/SOUL.md，把该专家的思路与流程模板融入研究方案与产出。
...
5. 执行（监督与登记）：指导 Genpilot 用 dcs_terminal_exec 在线跑分析；**长任务/并行 → dcs_offline_run（资源 "4c 16g" 或 "vf=32g,num_proc=8" 均可，插件本地校验并自动转换；会返回 task_id）或 dcs_parallel_run（返回 task_ids）**。离线任务法则：**镜像须为公共库注册路径 public-library/<镜像名>:latest（Docker Hub 短名如 ubuntu:24.04 会报 image_url不存在，投递前用 dcs_public_search 检索 resType=img 确认）；离线容器工作目录 = /data/work（非在线容器的 /work/{user}），/Files、share-data 只读；**挂载数据（-m /Files/...）在容器内需补全 /data/input/ 前缀访问**；复杂命令先写成脚本文件再 bash 执行，避免 -t/-c 被 CLI 当全局 flag**。**任务归属：投递前用 dcs_status 确认当前项目，任务应归属数据所在项目；DCS 项目间数据不共享，跨项目数据先用 dcs data copy --target-project <数据所在项目> 拷贝**。每个模块的每次运行：dcs_run_start 开启 → 监督执行 → dcs_run_update 更新状态/进度/文件清单（code/input/output）/notes，**并把离线任务返回的 task_id / task_ids 填入 dcs_task_ids 字段**（供「项目管理」窗口拉取实时费用）。
