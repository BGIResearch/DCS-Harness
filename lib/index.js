...
`;
  ...
  try {
    ...
    if (!r.ok) {
      return { ok: false, model: '', content: '', error: r.error || r.stderr || r.raw || 'terminal exec 失败（容器不可用或命令超时）' };
    }
...
f.close()`;
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_llm',
    description: '调用 DCS 系统 Genpilot LLM（对话模型，如 deepseek-v4-pro，自动选模型）做变异解读、文献综合、方案撰写、论文润色、任务失败归因等。在 DCS 在线容器内执行，鉴权由系统自动注入 LLM_API_KEY。json_mode=true 时请求结构化 JSON 输出并解析返回（JSON + 文本摘要双输出），适合失败诊断/参数预检等需要程序化处理的场景。注意：Genpilot chat 是对话模型，与 Genos 预测模型（VCF→RNA 信号，1.2B，需 dcs_api_key 配置）不同。',
    parameters: {
