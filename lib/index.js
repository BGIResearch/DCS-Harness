...
/** SAW 工具探测命令识别：命中 bc* 系列（bcSTAR/bcSaw/bcBarcode 等）或 SAW 流程脚本 + 探测意图（--help/-h/--version/-v）。
 *  支持 && / ; / || 链式命令（任一执行段命中即算）与 bash -c / source / cd / export 等包装前缀。 */
function isSawProbeCommand(cmd) {
  ...
  for (const seg of segs) {
    ...
    const isSawScript = /^SAW|^saw|^Stereo_Miner|^SC_Miner/i.test(tool);
    const probeFlag = /(--help|-h|--version|-v|\?)\b/.test(seg);
    if ((isBcTool || isSawScript) && probeFlag) return true;
  }
  return false;
}
...
function withSawEnv(cfg, cmd) {
  if (!isSawProbeCommand(cmd)) return cmd;
...
（执行阶段·全自动+自批评长循环）用户点「🚀 批准计划并自动执行」后（planStatus=approved）：按 dependsOn 顺序依次完成全部模块（dcs_run_start / dcs_run_update 登记每次运行与产物），**不再设任何审查节点、不暂停、不请示**——可自行决策的问题选合理默认并继续。**模块执行对话优先（默认范式）**：每个模块开始前**先调 dcs_module_consult（或 dcs_llm）通过 Genpilot 对话确定执行方案**（把模块目标 + 项目上下文交给对话，产出步骤/命令/预期产物），再按方案执行——执行动作优先选择对话给出的命令/流程；执行中失败、结果异常、参数不确定时**回到对话**（dcs_task_diagnose / dcs_llm / dcs_module_consult）分析调整，而不是闷头重试。**每个模块完成后强制「评估-调整循环」**（对标 Biomni 长 loop，纯执行阶段增强，不动批量审核制）：
...
4. 方案（第一优先级 = 交给 DCS Genpilot 对话执行）：**每个模块执行前先调 dcs_module_consult（或 dcs_llm）用 Genpilot 对话确定执行方案**——把模块目标、项目上下文（数据/依赖产物/可用能力）交给对话，产出结构化步骤（step/action/expect）；**分析计算优先按对话方案交 DCS Genpilot 在线容器交互完成**（dcs_terminal_exec / dcs_terminal_file，写 stepN_*.py 由 Genpilot 环境执行），或复用现有 WDL 流程（dcs_workflow_search / dcs_workflow_info / dcs_workflow_run）；**你只做方案设计与结果把关，不自上而下手写整套分析代码**。执行中问题（投递失败/结果异常/参数不确定）回到对话（dcs_task_diagnose / dcs_module_consult / dcs_llm）分析解决。仅在 Genpilot 无法覆盖的极少数场景，才用 dcs_audit_script 审计后自行补充。
