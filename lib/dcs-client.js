// dsh-dcs-cloud — DCS 云平台客户端（Host 侧，纯 Node）。
// 封装 BGI Research 发布的 `dcs` CLI（Go 二进制，https://github.com/BGIResearch/dcs_cli）：
//   - 二进制管理：检测/下载（含 SHA256 校验）到 ~/.dsh/dcs-bin/dcs
//   - PAT 登录：dcs auth login --token <PAT>
//   - 命令执行：execFile 运行 dcs 命令，统一 --output json，解析 {data,error,exit_code,...}
//
// 配置持久化于 ~/.dsh/dcs-cloud.json；PAT 交给 dcs 自身加密存储（~/.dcs/config.yaml），
// 本插件不落盘明文 PAT（仅运行时使用）。

import { readFileSync, writeFileSync, mkdirSync, existsSync, chmodSync, statSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { homedir, hostname } from 'node:os';
import { execFile } from 'node:child_process';
import { createHash, createCipheriv, createDecipheriv, randomBytes, scryptSync } from 'node:crypto';

// ---- dcs CLI 发行版（v1.2.0）----
// SHA256 取自官方发布页 v1.2.0 的 SHA256SUMS（2026-09-05 重新核实；linux-amd64 另经实测下载校验）。
// 注意：dcs 上游的 `--version` 恒输出 V1.0.0（未随版本号 bump），不可作为版本依据；以 SHA256 为准。
export const DCS_VERSION = 'v1.2.0';
export const DCS_RELEASE_BASE = 'https://github.com/BGIResearch/dcs_cli/releases/download/' + DCS_VERSION;
const RELEASE_SHA = {
  'dcs-linux-amd64': 'f4cf819d2ee899e7071f3e0dd70d54a32ecc574747ab2d0d2b1427c1d83c315b',
  'dcs-darwin-amd64': '81feb704e6030daf6a046a650735bd7dbbf911887cb99ec1464ac86aeaa72ed5',
  'dcs-darwin-arm64': 'd8b89e76bdbf424038be271c1d2d77d46e6c7f20a59739b8ccf270ee212ea15e',
  'dcs-darwin-universal': 'f30e7cef256fc26b39fe7bdff00b8bba636a7680f7c3b4934847714b8c93843c',
  'dcs.exe': '5cd97323901baeea04af5b77d3208594b1f6630be94a10199b8a7a193af4a228',
};

export const DEFAULT_CFG = {
  // dcs CLI 二进制路径或名称；默认 'dcs'（在 PATH 中查找，找不到则自动下载）。
  cliPath: 'dcs',
  // 自动下载开关：auto（找不到就下载）| never（只用已有二进制）
  autoInstall: 'auto',
  // dcs_pat（个人访问令牌）：仅用于直连 DCS 公共库 API（dcs_public_search）。
  // 由 dcs_login 写入本地配置，与 dcs 自身加密存储的 token 一致（同一 PAT）。
  pat: '',
  // DCS API base（公共库/元数据接口），默认中国主站。
  baseUrl: 'https://www.dcs.cloud',
  // API keys 映射：skill/服务名 → 其 API key（如 genos_vep → HG38_VCF_PREDICT_API_KEY）。
  // 由 dcs_api_key 工具管理，写入 ~/.dsh/dcs-cloud.json（600 权限）。
  // 模型/技能 API key 以此统一管理，使用时自动注入。
  apiKeys: {},
  // biolens MCP（OmicSeek 数据检索服务，DCS Genpilot biolens-search 同一后端）：dcs_biolens_search 宿主直连。
  // auth_key 经 apiKeys.biolens（dcs_api_key set biolens <key>，从 db.cngb.org/biolens 申请）或 env DCS_BIOLENS_KEY 提供；
  // 经 DCS Cloud 集成（Genpilot 容器对话）调用同一 MCP 时 list_dataset_files 会补 dcs_path（容器 /public 路径）。
  biolensUrl: 'https://db.cngb.org/biolens/mcp',
  // SAW（Stereo-seq Analysis Workflow）运行环境：投递 SAW 工具探测命令（如 bcSTAR --help）前
  // 自动加载运行环境，避免因缺 LD_LIBRARY_PATH/PATH 而误判工具不可用。
  // sawEnvMode: auto=自动（source <sawRoot>/env.sh 优先，失败回退 export LD_LIBRARY_PATH=<sawRoot>/anaconda/lib）
  //             | source=仅 source env.sh | ldpath=仅 export LD_LIBRARY_PATH | off=不自动加载。
  // sawRoot: SAW 安装根路径（含 env.sh 与 anaconda/lib），如 /opt/saw-8.2.2；留空用默认。
  sawEnvMode: 'auto',
  sawRoot: '/opt/saw-8.2.2',
};

// ---- 配置 ----
export function cfgPath() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dcs-cloud.json');
}

export function loadCfg() {
  let file = {};
  try { file = JSON.parse(readFileSync(cfgPath(), 'utf8')); } catch { /* 无文件 */ }
  let pat = process.env.DCS_PAT || file.pat || '';
  if (!pat && file.patEnc) pat = decryptPat(file.patEnc);
  return {
    ...DEFAULT_CFG,
    ...file,
    cliPath: process.env.DCS_CLI_PATH || file.cliPath || DEFAULT_CFG.cliPath,
    pat,
  };
}

export function saveCfg(next) {
  mkdirSync(dirname(cfgPath()), { recursive: true });
  const toWrite = { ...next };
  // PAT 不落盘明文（Genos #1 / doc 8.4）：写入前用 AES-256-GCM 加密到 patEnc，明文 pat 置空。
  if (typeof toWrite.pat === 'string' && toWrite.pat) {
    const enc = encryptPat(toWrite.pat);
    toWrite.patEnc = enc || null;
    toWrite.pat = '';
  }
  writeFileSync(cfgPath(), JSON.stringify(toWrite, null, 2));
  // 配置文件含加密 PAT，仍收紧为仅本用户可读写。
  try { chmodSync(cfgPath(), 0o600); } catch { /* 平台不支持时忽略 */ }
}

// ---- PAT 静态加密（AES-256-GCM，密钥由 hostname + DSH_HOME 派生；跨机器/改装不共享） ----
// 密钥非「秘密分发」，仅用于避免明文落盘：拿到本机文件即可解密；相比明文显著降险。
const PAT_KEY_SALT = 'dsh-dcs-cloud::pat-v1';
function patKey() {
  const secret = hostname() + '|' + (process.env.DSH_HOME || join(homedir(), '.dsh')) + '|dsh-dcs-cloud-pat';
  return scryptSync(secret, PAT_KEY_SALT, 32);
}
function encryptPat(pat) {
  try {
    const iv = randomBytes(12);
    const cipher = createCipheriv('aes-256-gcm', patKey(), iv);
    const data = Buffer.concat([cipher.update(String(pat), 'utf8'), cipher.final()]);
    return { iv: iv.toString('base64'), tag: cipher.getAuthTag().toString('base64'), data: data.toString('base64') };
  } catch { return null; }
}
function decryptPat(enc) {
  try {
    const decipher = createDecipheriv('aes-256-gcm', patKey(), Buffer.from(enc.iv, 'base64'));
    decipher.setAuthTag(Buffer.from(enc.tag, 'base64'));
    return Buffer.concat([decipher.update(Buffer.from(enc.data, 'base64')), decipher.final()]).toString('utf8');
  } catch { return ''; }
}

// ---- 平台 / 二进制 ----
export function platformBinary() {
  const p = process.platform; // darwin | linux | win32
  const a = process.arch; // arm64 | x64 | ...
  if (p === 'darwin' && a === 'arm64') return 'dcs-darwin-arm64';
  if (p === 'darwin' && a === 'x64') return 'dcs-darwin-amd64';
  if (p === 'darwin') return 'dcs-darwin-universal';
  if (p === 'linux' && a === 'x64') return 'dcs-linux-amd64';
  if (p === 'win32') return 'dcs.exe';
  return null;
}

export function managedBinDir() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dcs-bin');
}
export function managedBinPath() {
  const name = platformBinary();
  return name ? join(managedBinDir(), name) : null;
}

function sha256(buf) {
  return createHash('sha256').update(buf).digest('hex');
}

/** 下载 dcs 二进制并校验 SHA256，返回落盘路径；失败抛错。 */
export async function downloadDcsBinary(signal) {
  const name = platformBinary();
  if (!name) throw new Error('当前平台无可用 dcs 二进制（' + process.platform + '/' + process.arch + '）');
  const url = DCS_RELEASE_BASE + '/' + name;
  const out = managedBinPath();
  mkdirSync(managedBinDir(), { recursive: true });
  const t = AbortSignal.timeout(300000);
  const sig = signal ? AbortSignal.any([signal, t]) : t;
  const res = await fetch(url, { signal: sig });
  if (!res.ok) throw new Error('下载失败: HTTP ' + res.status);
  const buf = Buffer.from(await res.arrayBuffer());
  const expect = RELEASE_SHA[name];
  const got = sha256(buf);
  if (expect && got !== expect) throw new Error('SHA256 校验失败：期望 ' + expect + '，实际 ' + got);
  writeFileSync(out, buf);
  chmodSync(out, 0o755);
  return out;
}

function isExecutable(p) {
  return new Promise((resolve) => {
    try {
      execFile(p, ['--version'], { timeout: 20000 }, (err) => resolve(!err));
    } catch { resolve(false); }
  });
}

// 二进制解析缓存：每次 runDcs 都解析很贵（每个候选 spawn `--version` 探测，
// PATH 无 dcs 时每个候选要等 20s 超时）。按「配置指纹 + managed 二进制 mtime/size」
// 做 key，命中且 30s 内直接复用；探测失败（包括未安装）也缓存，避免高频工具
// （如 5s 轮询费用/状态）重复踩超时。managed 二进制被替换时 key 变化自动失效。
const BIN_CACHE_TTL = 30 * 1000;
const binResolveCache = new Map(); // key -> { at, result }（result 为 {path, source} 或 {error}）

function binFingerprint(cfg) {
  const managed = managedBinPath();
  let m = 'none';
  if (managed) {
    try {
      const st = statSync(managed);
      m = st.mtimeMs + ':' + st.size;
    } catch { /* 未下载 */ }
  }
  return (cfg.cliPath || 'dcs') + '|' + (cfg.autoInstall || 'auto') + '|' + m;
}

/** 解析可用二进制路径：cliPath → PATH → 已下载 → 自动下载（带 30s 结果缓存）。 */
export async function resolveDcsBinary(cfg, signal) {
  const key = binFingerprint(cfg);
  const hit = binResolveCache.get(key);
  if (hit && Date.now() - hit.at < BIN_CACHE_TTL) {
    if (hit.result.path) return hit.result;
    throw new Error(hit.result.error);
  }
  const candidates = [];
  if (cfg.cliPath && cfg.cliPath !== 'dcs') candidates.push(cfg.cliPath);
  candidates.push('dcs'); // PATH 查找
  const managed = managedBinPath();
  if (managed) candidates.push(managed);

  // 并行探测全部候选，取「优先级最高」的可用者：避免 PATH 无 dcs 时串行等
  // 每个候选的 20s 超时（最坏 3×20s=60s 才轮到 managed/下载）。
  const probes = await Promise.all(candidates.map(async (c) => ({ c, ok: await isExecutable(c) })));
  for (const p of probes) {
    if (p.ok) {
      const result = { path: p.c, source: p.c === managed ? 'managed' : 'configured' };
      binResolveCache.set(key, { at: Date.now(), result });
      return result;
    }
  }
  if (cfg.autoInstall !== 'never') {
    try {
      const p = await downloadDcsBinary(signal);
      const result = { path: p, source: 'downloaded' };
      binResolveCache.set(key, { at: Date.now(), result });
      return result;
    } catch (e) {
      const err = '未找到可用 dcs CLI（下载也失败: ' + String(e && e.message || e) + '）';
      binResolveCache.set(key, { at: Date.now(), result: { error: err } });
      throw new Error(err);
    }
  }
  const err = '未找到 dcs CLI，且 autoInstall=never。请用 dcs_configure 指定 cliPath 或安装 dcs。';
  binResolveCache.set(key, { at: Date.now(), result: { error: err } });
  throw new Error(err);
}

// ---- 命令执行 ----

/**
 * 运行 dcs 命令（统一追加 --output json）。
 * @param {string[]} args 不含二进制名的参数（如 ['data','ls','/Files']）
 * @param {object} opts { signal, timeoutMs, output: 'json'|'table'|'ndjson', cwd, env }
 */
export async function runDcs(cfg, args, opts) {
  const o = opts || {};
  const bin = await resolveDcsBinary(cfg, o.signal);
  const out = o.output || 'json';
  const fullArgs = [...args, '--output', out];
  if (o.noHistory !== false) fullArgs.push('--no-history');
  const result = await new Promise((resolve) => {
    let child;
    try {
      child = execFile(bin.path, fullArgs, {
        timeout: o.timeoutMs || 180000,
        maxBuffer: 40 * 1024 * 1024,
        cwd: o.cwd,
        env: { ...process.env, ...(o.env || {}) },
      }, (err, stdout, stderr) => {
        resolve({ err, stdout: String(stdout || ''), stderr: String(stderr || '') });
      });
      if (o.signal) {
        o.signal.addEventListener('abort', () => { try { child.kill('SIGKILL'); } catch {} }, { once: true });
      }
    } catch (e) {
      resolve({ err: e, stdout: '', stderr: String(e.message || e) });
    }
  });

  let parsed = null;
  try { parsed = JSON.parse(result.stdout); } catch { /* 非 JSON（table 等） */ }
  // dcs 失败响应里 error 是嵌套对象 {type, detail:{business_code,message}, hint, retryable}，
  // 统一压平为可读字符串，避免上层对非字符串调用 slice 等崩溃。
  let errText = '';
  if (parsed && parsed.error !== null && parsed.error !== undefined && parsed.error !== '') {
    const e = parsed.error;
    if (typeof e === 'string') errText = e;
    else if (typeof e === 'object') {
      const detail = (e.detail && typeof e.detail === 'object') ? e.detail : {};
      errText = String(detail.message || e.hint || e.type || JSON.stringify(e));
      if (detail.business_code) errText += '（错误码 ' + detail.business_code + '）';
      // 透传完整诊断信息：hint / retryable / metadata，避免「系统内部错误」黑盒
      if (detail.message && e.hint) errText += '；提示：' + String(e.hint);
      if (e.retryable === true || detail.retryable === true) errText += '（可重试）';
      if (parsed.metadata && typeof parsed.metadata === 'object' && Object.keys(parsed.metadata).length) {
        errText += '；metadata: ' + JSON.stringify(parsed.metadata).slice(0, 300);
      }
      // 笼统错误（99999 系统内部错误 / 81201 等）附 stderr 末尾，尽量带出底层 api_msg
      const generic = /系统内部错误|internal error/i.test(errText) || /（错误码 (99999|81201)）/.test(errText);
      const stderrTail = String(result.stderr || '').trim().slice(-500);
      if (generic && stderrTail) errText += '；stderr: ' + stderrTail;
    } else errText = String(e);
    // 信封级 request_id，便于 dcs history get <request_id> 反查底层响应
    if (parsed.request_id) errText += ' [request_id: ' + parsed.request_id + ']';
  }
  return {
    ok: !result.err && parsed !== null && (parsed.exit_code === 0 || parsed.exit_code === undefined),
    exit_code: parsed ? parsed.exit_code : (result.err ? (result.err.code || 1) : 0),
    data: parsed ? parsed.data : null,
    error: errText,
    message: parsed ? parsed.message : '',
    raw: result.stdout,
    stderr: result.stderr,
    binary: bin.path,
    source: bin.source,
  };
}

/** 登录（dcs auth login --token <PAT>）。 */
export async function dcsLogin(cfg, token, opts) {
  return runDcs(cfg, ['auth', 'login', '--token', String(token || '')], { ...(opts || {}), timeoutMs: 60000 });
}

/** 通用状态快照（当前用户/区域/项目）。 */
export async function dcsStatus(cfg, opts) {
  const o = opts || {};
  const [project, region, config] = await Promise.all([
    runDcs(cfg, ['project', 'current'], { ...o, timeoutMs: 60000 }).catch(() => null),
    runDcs(cfg, ['region', 'current'], { ...o, timeoutMs: 60000 }).catch(() => null),
    runDcs(cfg, ['config', 'show'], { ...o, timeoutMs: 60000 }).catch(() => null),
  ]);
  // dcs CLI v1.2.0 起字段漂移：project current 回 project_name（旧 current_project_name）且不再含 username；
  // region current 也不含 username。此处从 config show 与字段别名归一化，使上层稳定读取。
  // config show 的 token 不落地/不外泄，仅取 username/current_user。
  const cdata = (config && config.data) || {};
  const cuser = cdata.username || cdata.current_user || '';
  if (project && project.data) {
    project.data.current_project_name = project.data.current_project_name || project.data.project_name || '';
    project.data.username = project.data.username || cuser;
  }
  if (region && region.data) {
    region.data.username = region.data.username || cuser;
  }
  return { project, region };
}

// ---- 公共库（publib）直连 API ----
// dcs CLI v1.1.0 未暴露「公共数据」命令，但公共库有独立 REST API：
//   GET {baseUrl}/api/publib/dcsRes/search?name=<关键词>&page=&pageSize=
// 返回 resources（resType: dataset/workflow/img/proj/ai_model 等）。需 dcs_pat + X-Client-App。

/** 搜索 DCS 公共库（公共数据/流程/项目/AI模型）。 */
export async function publicSearch(cfg, name, opts) {
  const o = opts || {};
  const base = (cfg.baseUrl || DEFAULT_CFG.baseUrl).replace(/\/$/, '');
  const pat = cfg.pat || '';
  if (!pat) throw new Error('未配置 PAT。请先 dcs_login（PAT 会写入本地配置）再检索公共库。');
  const params = new URLSearchParams();
  if (name) params.set('name', String(name));
  params.set('page', String(o.page || 1));
  params.set('pageSize', String(o.pageSize || 30));
  const t = AbortSignal.timeout(o.timeoutMs || 60000);
  const sig = o.signal ? AbortSignal.any([o.signal, t]) : t;
  const res = await fetch(base + '/api/publib/dcsRes/search?' + params.toString(), {
    method: 'GET',
    headers: { Authorization: 'Bearer ' + pat, 'X-Client-App': 'dcs-cloud-cli' },
    signal: sig,
  });
  const text = await res.text();
  let data = null;
  try { data = JSON.parse(text); } catch { /* 非 JSON */ }
  if (!res.ok || (data && data.code && data.code !== 0)) {
    const msg = data && data.msg ? data.msg : ('HTTP ' + res.status);
    throw new Error('公共库检索失败: ' + msg);
  }
  const recs = (data && data.data && data.data.records) || [];
  return {
    total: (data && data.data && data.data.total) || recs.length,
    records: recs.map((r) => ({
      name: r.name, resType: r.resType, intro: (r.intro || '').slice(0, 200),
      zone: r.srcZone, id: r.rawId, tags: r.tags || [], omics: r.omics || [],
      officialTag: r.official_tag || '', creator: r.createdBy,
    })),
  };
}

// 注：Genpilot LLM 与 Genos 模型为 DCS 系统自带、自动鉴权、模型自动选择，
// 由 dcs_llm 工具在在线容器内调用（读系统注入的 LLM_API_KEY / DCS_X_ACCESS_TOKEN），无需本地配置 key。
