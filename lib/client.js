...
function FilesView(props) {
  ...
  function listDir(p, quiet) {
    if (!p) return;
    if (!quiet) { setBusy(true); setErr(""); setPrev(null); }
    api("/v2/files/list?path=" + encodeURIComponent(p)).then(function (d) {
      if (!quiet) setBusy(false);
      if (!d || !d.ok) { if (!quiet) setErr((d && d.error) || "列目录失败"); return; }
      setCur(d.dir || p); setEntries(d.entries || []);
    }).catch(function (e) { if (!quiet) setBusy(false); if (!quiet) setErr(String((e && e.message) || e)); });
  }
  ...
  React.useEffect(function () {
    api("/v2/files/status").then(function (d) {
      ...
      setInfo(d);
      // 首次进入 / 项目切换后：自动回到该项目容器工作目录并列出（无需手动刷新）
      if (!cur || changed) setCur(d.home || "/work");
  ...
  React.useEffect(function () {
    var timer = setInterval(function () {
      if (cur && !busy && !prevBusy) listDir(cur, true);
    }, 15000);
