...
function FilesView(props) {
  ...
  var curState = React.useState(""); var cur = curState[0]; var setCur = curState[1];
  var entriesState = React.useState([]); var entries = entriesState[0]; var setEntries = entriesState[1];
  var busyState = React.useState(false); var busy = busyState[0]; var setBusy = busyState[1];
  var errState = React.useState(""); var err = errState[0]; var setErr = errState[1];
  var prevState = React.useState(null); var prev = prevState[0]; var setPrev = prevState[1];
  ...
  function listDir(p, mode) {
    ...
    api("/v2/files/list?path=" + encodeURIComponent(p)).then(function (d) {
      ...
      var dir = d.dir || p;
      setCur(dir); setEntries(d.entries || []);
      // 记住当前打开的目录（按会话），切走再回来恢复到同一路径
      dcsFilesSave(sessionId, { project: lastProjRef.current || "", path: dir });
    }).catch(function (e) {
  ...
  React.useEffect(function () {
    api("/v2/files/status").then(function (d) {
      ...
      setInfo(d);
      // 记住上次打开的路径：同一项目切回 tab 时恢复到原目录（静默刷新）；仅首次/换项目才回工作目录
      var target = (!changed && st && st.project === proj && st.path) ? st.path : (d.home || "/work");
      dcsFilesSave(sessionId, { project: proj, path: target });
      if (cur !== target) setCur(target);
    }).catch(function (e) { setErr(String((e && e.message) || e)); });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionId]);

  // 目录变化 → 自动列出（首次进入 / 导航 / 回到工作目录都覆盖）
  React.useEffect(function () {
    if (cur) listDir(cur, "auto");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cur]);

  // 常驻静默自动刷新（15s）：切回本 tab 时内容始终是最新，不必每次手动点「刷新」；忙碌/预览时跳过
  React.useEffect(function () {
    var timer = setInterval(function () {
      if (cur && !busy && !prevBusy) listDir(cur, "poll");
    }, 30000);
  ...
  children.push(h("div", { style: { marginTop: 8, maxHeight: 260, overflowY: "auto", border: "1px solid rgba(128,128,128,.15)", borderRadius: 6 } },
  ...
  if (prev) {
    var prevBody = null;
    if (prev.type === "image") {
      prevBody = h("div", { style: { marginTop: 8, maxHeight: 420, overflow: "auto", textAlign: "center" } }, h("img", { src: prev.content, style: { maxWidth: "100%", borderRadius: 6 } }));
    } else if (prev.type === "text") {
      prevBody = h("pre", { style: { marginTop: 8, maxHeight: 420, overflow: "auto", whiteSpace: "pre-wrap", wordBreak: "break-all", fontSize: 11, fontFamily: "ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" } }, prev.content);
    } else {
      prevBody = h("div", { style: { marginTop: 8, opacity: .8 } }, prev.type === "pdf" ? "PDF 文件请点「下载」后用本地阅读器打开。" : ("该文件为二进制/过大（" + dcsFmtSize(prev.size) + "），不支持在线预览——点「下载」取回本机查看。"));
    }
    children.push(h("div", { style: { marginTop: 8, border: "1px solid rgba(15,98,254,.35)", borderRadius: 8, padding: "8px 10px" } },
      h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 } },
        ...
        h("span", { style: { display: "flex", gap: 6 } },
          ...
          h("button", { style: btnStyle, onClick: function () { setPrev(null); } }, "✕ 关闭"))),
      prevBody));
  }
