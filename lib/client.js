...
function layoutWorkflow(modules, plan) {
  ...
  // 网格布局：最多 4 列
  var cols = Math.min(4, order.length || 1);
  var rows = Math.ceil(order.length / cols);
  var cellW = 220, cellH = 90, padX = 40, padY = 30;
  var nodes = [];
  for (var n = 0; n < order.length; n++) {
    ...
    nodes.push({
      ...
      x: padX + col * (cellW + padX),
      y: padY + row * (cellH + padY),
      w: cellW,
      h: cellH,
    });
  }
  // 边：按顺序连接（plan.stepIds 或 创建顺序）
  var edges = [];
...
function WorkflowCanvas(props) {
  ...
  var plan = props.plan;
  var layout = layoutWorkflow(modules, plan);
  ...
  return h("div", { style: { border: "1px solid rgba(128,128,128,.25)", borderRadius: 12, overflow: "hidden", background: "rgba(128,128,128,.03)", marginBottom: 12 } },
    ...
    h("svg", {
      ...
      edges.map(function (edge, ei) {
        ...
        var mx = (x1 + x2) / 2;
        return h("path", { key: "e" + ei, d: "M" + x1 + "," + y1 + " C" + mx + "," + y1 + " " + mx + "," + y2 + " " + x2 + "," + y2, fill: "none", stroke: "rgba(128,128,128,.4)", strokeWidth: 2, markerEnd: "url(#wf-arrow)" });
      }),
      // 节点
