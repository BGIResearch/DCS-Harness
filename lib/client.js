...
function injectCss() {
  ...
  style.textContent =
    ...
    /* 执行轨迹：OAA 三段式 + 自评 verdict（Biomni 式 ReAct 自批评） */
    ".dcs2 .run-oaa{margin-top:8px;border-left:3px solid rgba(0,161,176,.55);padding:6px 10px;background:rgba(0,161,176,.07);border-radius:0 8px 8px 0;font-size:12.5px}" +
    ".dcs2 .oaa-row{display:flex;gap:7px;margin:3px 0;line-height:1.6;align-items:flex-start}" +
    ".dcs2 .oaa-k{flex:none;font-weight:700;font-size:10.5px;letter-spacing:.4px;padding:1px 7px;border-radius:999px;margin-top:2px}" +
    ".dcs2 .oaa-k.obs{background:rgba(100,181,205,.18);color:#64b5cd}" +
    ".dcs2 .oaa-k.ass{background:rgba(245,165,36,.18);color:#f5a524}" +
    ".dcs2 .oaa-k.act{background:rgba(34,160,107,.18);color:#22a06b}" +
    ".dcs2 .oaa-t{min-width:0;white-space:pre-wrap}" +
    ".dcs2 .verdict{font-size:10.5px;font-weight:700;padding:1px 8px;border-radius:999px;border:1px solid}" +
    ".dcs2 .verdict.pass{color:#22a06b;border-color:#22a06b}" +
    ".dcs2 .verdict.revise{color:#f5a524;border-color:#f5a524}" +
    ".dcs2 .verdict.rerun{color:#e5484d;border-color:#e5484d}" +
...
var VERDICT_TXT = { pass: "✅ 通过", revise: "🔄 需调整", rerun: "🔁 需重跑" };
...
function RunItem(props) {
  ...
  var hasMore = hasFiles || (run.dcsTaskIds && run.dcsTaskIds.length) || run.notes || run.oaa || run.selfReview;
  ...
  return h("div", { className: "run " + run.status },
    h("div", { className: "run-head", onClick: function () { if (hasMore) setOpen(!open); } },
      ...
      run.selfReview && run.selfReview.verdict ? h("span", { className: "verdict " + run.selfReview.verdict, title: "自评：" + (run.selfReview.evidence || "") }, VERDICT_TXT[run.selfReview.verdict] || run.selfReview.verdict) : null,
    ...
    open ? h("div", null,
      ...
      run.oaa && (run.oaa.observation || run.oaa.assessment || run.oaa.action) ? h("div", { className: "run-oaa" },
        run.oaa.observation ? h("div", { className: "oaa-row" }, h("span", { className: "oaa-k obs" }, "观察"), h("span", { className: "oaa-t" }, run.oaa.observation)) : null,
        run.oaa.assessment ? h("div", { className: "oaa-row" }, h("span", { className: "oaa-k ass" }, "评估"), h("span", { className: "oaa-t" }, run.oaa.assessment)) : null,
        run.oaa.action ? h("div", { className: "oaa-row" }, h("span", { className: "oaa-k act" }, "行动"), h("span", { className: "oaa-t" }, run.oaa.action)) : null
      ) : null,
      run.selfReview && (run.selfReview.evidence || (run.selfReview.risks && run.selfReview.risks.length) || run.selfReview.nextAction) ? h("div", { className: "run-meta" },
        run.selfReview.evidence ? h("span", null, "自评依据：" + run.selfReview.evidence) : null,
        run.selfReview.risks && run.selfReview.risks.length ? h("span", null, "风险：" + run.selfReview.risks.join("；")) : null,
        run.selfReview.nextAction ? h("span", null, "下一步：" + run.selfReview.nextAction) : null
      ) : null,
...
// ===== 执行看门狗状态条（防意外暂停）=====
// 已批准且未完成的项目：显示守护状态（守护中 / 已自动恢复 N 次 / 需人工介入），
// 让用户直观看到「意外停滞自动恢复」机制在工作。
function WatchdogBar(props) {
  var wd = (props.project && props.project.watchdog) || {};
  var approved = props.project && props.project.plan && props.project.plan.planStatus === 'approved';
  if (!approved || !wd.enabled) return null;
  var blocked = wd.blockedAt > 0;
  var style = blocked
    ? { borderLeft: "3px solid #e5484d", background: "rgba(229,72,77,.08)", color: "#e5484d" }
    : wd.wakes > 0
      ? { borderLeft: "3px solid #f5a524", background: "rgba(245,165,36,.08)", color: "#c99700" }
      : { borderLeft: "3px solid #22a06b", background: "rgba(34,160,107,.07)", color: "#1b7f56" };
  var label = blocked
    ? "⛔ 执行守护：多次自动恢复无进展，需人工介入"
    : wd.wakes > 0
      ? "🛡️ 执行守护：已自动恢复 " + wd.wakes + " 次（意外停滞自动续跑）"
      : "🛡️ 执行守护：运行中（LLM/连接异常导致停滞时自动续跑）";
  return h("div", { style: Object.assign({ fontSize: 11.5, padding: "6px 10px", borderRadius: 8, marginBottom: 10 }, style) },
    label,
    wd.lastReason ? h("span", { style: { opacity: .75, marginLeft: 6 } }, "· " + wd.lastReason) : null
  );
}
...
var SECTION_META = [
  ...
  { key: "trajectory", label: "执行轨迹", icon: "🔁", en: "ReAct Trajectory", color: "#00a1b0" },
];
...
// ================= 项目文件 tab（浏览当前项目容器 /work 与 /public，小文件在线预览、大文件下载）
// 状态保持：路径/列表/预览/滚动位置放模块级内存（FilesMem），切 tab 重建时同步恢复，不做异步重载。
function dcsFmtSize(n) {
...
function dcsFmtTime(ts) {
  if (!ts) return "";
  try { return new Date(ts * 1000).toLocaleString("zh-CN", { hour12: false }); } catch (e) { return ""; }
}
function dcsJoin(cur, name) {
  return cur === "/" ? "/" + name : String(cur || "").replace(/\/+$/, "") + "/" + name;
}
function dcsSegs(p) { return String(p || "").split("/").filter(Boolean); }
function dcsUp(p) { var s = dcsSegs(p); s.pop(); return s.length ? "/" + s.join("/") : "/"; }
function dcsFilesKey(sid) { return "dcs2-files-" + (sid || "default"); }
function dcsFilesLoad(sid) {
  try { var v = JSON.parse(window.localStorage.getItem(dcsFilesKey(sid)) || "null"); if (v && typeof v === "object" && typeof v.path === "string") return v; } catch (e) {}
  return {};
}
function dcsFilesSave(sid, o) {
  try { window.localStorage.setItem(dcsFilesKey(sid), JSON.stringify(o)); } catch (e) {}
}
var FilesMem = {};
function filesMem(sid) {
  var k = sid || "default";
  if (!FilesMem[k]) FilesMem[k] = { path: "", entries: [], loaded: false, project: "", preview: null, scroll: { list: 0, prev: 0 } };
  return FilesMem[k];
}
function FilesView(props) {
  var sessionId = (props && props.sessionId) ? props.sessionId : "";
  var mem = filesMem(sessionId);
  var infoState = React.useState(null); var info = infoState[0]; var setInfo = infoState[1];
  var curState = React.useState(mem.path || ""); var cur = curState[0]; var setCur = curState[1];
  var entriesState = React.useState(mem.entries || []); var entries = entriesState[0]; var setEntries = entriesState[1];
  var busyState = React.useState(false); var busy = busyState[0]; var setBusy = busyState[1];
  var errState = React.useState(""); var err = errState[0]; var setErr = errState[1];
  var prevState = React.useState(mem.preview); var prev = prevState[0]; var setPrev = prevState[1];
  var prevBusyState = React.useState(false); var prevBusy = prevBusyState[0]; var setPrevBusy = prevBusyState[1];
  var jumpState = React.useState(""); var jump = jumpState[0]; var setJump = jumpState[1];
  var lastProjRef = React.useRef("");
  var listRef = React.useRef(null);
  var prevRef = React.useRef(null);

  var rootStyle = {
    border: "1px solid rgba(128,128,128,.25)", borderRadius: 10, margin: 8, padding: "10px 12px",
    fontSize: 12, color: "inherit", background: "rgba(128,128,128,.04)", lineHeight: 1.6,
  };
  var barStyle = { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" };
  var btnStyle = {
    padding: "3px 10px", borderRadius: 6, border: "1px solid rgba(128,128,128,.45)",
    background: "transparent", color: "inherit", cursor: "pointer", fontSize: 12,
  };
  var rowStyle = { display: "flex", alignItems: "center", gap: 10, padding: "3px 6px", borderRadius: 6, cursor: "default" };
  var nameStyle = { flex: "1 1 220px", minWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
  var fnameBtn = { color: "inherit", cursor: "pointer", background: "none", border: "none", padding: 0, font: "inherit", textAlign: "left", maxWidth: "100%" };

  function listDir(p, mode) {
    if (!p) return;
    var quiet = mode === "poll";
    var auto = mode === "auto";
    if (!quiet && !auto) setBusy(true);
    if (!auto) setErr("");
    api("/v2/files/list?path=" + encodeURIComponent(p)).then(function (d) {
      if (!quiet && !auto) setBusy(false);
      if (!d || !d.ok) { if (mode !== "poll") setErr((d && d.error) || "列目录失败"); return; }
      var dir = d.dir || p;
      var list = d.entries || [];
      mem.path = dir; mem.entries = list; mem.loaded = true; mem.project = mem.project || lastProjRef.current || "";
      setCur(dir); setEntries(list);
      dcsFilesSave(sessionId, { project: lastProjRef.current || mem.project, path: dir });
    }).catch(function (e) {
      if (mode !== "poll") { if (!quiet && !auto) setBusy(false); setErr(String((e && e.message) || e)); }
    });
  }

  // 挂载：只校验状态/项目；同一项目且内存已有内容则完全不动（不重载、不闪动）
  React.useEffect(function () {
    api("/v2/files/status").then(function (d) {
      if (!d || !d.ok) { setErr((d && d.error) || "状态读取失败"); return; }
      var proj = d.project || "";
      var changed = !!(lastProjRef.current && proj && lastProjRef.current !== proj);
      lastProjRef.current = proj;
      setInfo(d);
      if (!mem.loaded || changed || mem.project !== proj) {
        var st = dcsFilesLoad(sessionId);
        mem.project = proj;
        if (changed) { mem.entries = []; mem.preview = null; mem.scroll = { list: 0, prev: 0 }; setEntries([]); setPrev(null); }
        mem.path = "";
        var target = (!changed && st.project === proj && st.path) ? st.path : (d.home || "/work");
        dcsFilesSave(sessionId, { project: proj, path: target });
        setCur(target);
      } else {
        mem.project = proj;
      }
  ...
  // 目录变化：仅在内存里没有该目录内容时才真正去列目录（切回原目录/恢复时不重载）
  React.useEffect(function () {
    if (cur && !(mem.loaded && mem.path === cur)) listDir(cur, "auto");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cur]);

  // 后台静默刷新（30s）：仅在当前目录已加载时更新列表，不打扰预览/滚动
  React.useEffect(function () {
    var timer = setInterval(function () {
      if (cur && !busy && !prevBusy && mem.loaded) listDir(cur, "poll");
    }, 30000);
    return function () { clearInterval(timer); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cur, busy, prevBusy]);

  // 每次渲染后恢复记忆的滚动位置（切回时无感回到原位置）
  React.useEffect(function () {
    if (listRef.current && mem.scroll.list > 0) { try { listRef.current.scrollTop = mem.scroll.list; } catch (e) {} }
    if (prevRef.current && mem.scroll.prev > 0) { try { prevRef.current.scrollTop = mem.scroll.prev; } catch (e) {} }
  });
  ...
  React.useEffect(function () {
    return function () {
      try {
        if (listRef.current) mem.scroll.list = listRef.current.scrollTop || 0;
        if (prevRef.current) mem.scroll.prev = prevRef.current.scrollTop || 0;
      } catch (e) {}
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  function openItem(it) {
    var full = dcsJoin(cur, it.name);
    if (it.type === "dir") { setCur(full); return; }
    setErr(""); setPrevBusy(true);
    api("/v2/files/read?path=" + encodeURIComponent(full)).then(function (d) {
      setPrevBusy(false);
      if (d && d.ok) { setPrev(d); mem.preview = d; mem.scroll.prev = 0; }
      else setErr((d && d.error) || "预览失败");
    }).catch(function (e) { setPrevBusy(false); setErr(String((e && e.message) || e)); });
  }
  function closePreview() {
    setPrev(null); mem.preview = null; mem.scroll.prev = 0;
  }
  function downloadFile(it) {
    var full = dcsJoin(cur, it.name);
    var a = document.createElement("a");
    a.href = "/api/dcs-cloud/v2/files/download?path=" + encodeURIComponent(full);
    a.download = it.name;
    document.body.appendChild(a); a.click();
    setTimeout(function () { try { document.body.removeChild(a); } catch (e) {} }, 60);
  }
  function goJump() {
    var p = String(jump || "").trim();
    if (!p) return;
    setErr(""); setCur(p);
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    if (entries[i].type === "dir") dirs.push(entries[i]); else files.push(entries[i]);
  }
  dirs.sort(function (a, b) { return String(a.name).localeCompare(b.name); });
  files.sort(function (a, b) { return String(a.name).localeCompare(b.name); });
  var rows = dirs.concat(files);
  var segs = dcsSegs(cur);
  var bread = [];
  bread.push(h("span", { key: "root", style: { cursor: "pointer", color: "#0f62fe", fontWeight: 600 }, onClick: function () { setCur("/"); } }, "/"));
  var acc = "";
  for (var b = 0; b < segs.length; b++) {
    acc += "/" + segs[b];
    bread.push(h("span", { key: "sep" + b, style: { color: "rgba(128,128,128,.7)" } }, "/"));
    (function (prefix, idx) {
      bread.push(h("span", { key: "seg" + idx, style: { cursor: "pointer", color: "#0f62fe" }, onClick: function () { setCur(prefix); } }, segs[idx]));
    })(acc, b);
  }
  var children = [];
  children.push(h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap", gap: 6 } },
    h("span", { style: { fontWeight: 700 } }, "📁 项目文件" + (info ? (" · " + ((info.projectName || info.project) || "") + " · " + (info.username || "") + "@" + (info.region || "")) : "")),
    h("button", { style: btnStyle, onClick: function () { listDir(cur); } }, "🔄 刷新")));
  children.push(h("div", { style: barStyle },
    h("span", { style: { opacity: .8 } }, "路径"),
    h("div", { style: { flex: 1, minWidth: 200, display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap", padding: "2px 6px", border: "1px solid rgba(128,128,128,.25)", borderRadius: 6, background: "rgba(128,128,128,.05)" } }, bread)));
  children.push(h("div", { style: Object.assign({}, barStyle, { marginTop: 6 }) },
    h("input", {
      style: { flex: 1, minWidth: 200, padding: "3px 8px", borderRadius: 6, border: "1px solid rgba(128,128,128,.35)", background: "transparent", color: "inherit", fontSize: 12 },
      placeholder: "输入容器绝对路径跳转（仅 /work /public /home），如 /public/database/CNGBdb/pub/SciRAID/stomics",
      value: jump, onChange: function (e) { setJump(e.target.value); },
      onKeyDown: function (e) { if (e.key === "Enter") goJump(); },
    }),
    h("button", { style: btnStyle, onClick: goJump }, "跳转"),
    h("button", { style: btnStyle, onClick: function () { if (cur && cur !== "/") setCur(dcsUp(cur)); } }, "⬆ 上一级"),
    h("button", { style: btnStyle, onClick: function () { setCur(info ? info.home : "/work"); } }, "🏠 工作目录"),
    info ? h("button", { style: btnStyle, title: "BGI 中心公共库挂载（时空片区）", onClick: function () { setCur("/public/database/CNGBdb/pub/SciRAID/stomics"); } }, "🧬 公共时空数据") : null,
    info ? h("button", { style: btnStyle, title: "公共库根", onClick: function () { setCur("/public"); } }, "📚 /public") : null));
  var listInner = [];
  if (cur && cur !== "/") {
    listInner.push(h("div", { key: "..", style: Object.assign({}, rowStyle, { cursor: "pointer", fontWeight: 600 }), onClick: function () { setCur(dcsUp(cur)); } },
      h("span", {}, "⬆"), h("span", { style: nameStyle }, "..")));
  }
  rows.forEach(function (it) {
    ...
    var actions = [];
    if (!isDir) {
      actions.push(h("button", { key: "p", style: Object.assign({}, btnStyle, { padding: "1px 8px", fontSize: 11 }), onClick: function () { openItem(it); } }, "预览"));
      actions.push(h("button", { key: "d", style: Object.assign({}, btnStyle, { padding: "1px 8px", fontSize: 11 }), title: "大文件下载（容器→本机）", onClick: function () { downloadFile(it); } }, "下载"));
    }
    listInner.push(h("div", {
      key: it.name, style: Object.assign({}, rowStyle, { background: "transparent" }),
      onClick: function () { if (isDir) setCur(full); },
      onDoubleClick: function () { openItem(it); },
    },
      h("span", { style: { flex: "none", width: 18 } }, isDir ? "📁" : "📄"),
      h("span", { style: nameStyle, title: full },
        isDir
          ? h("a", { style: fnameBtn, onClick: function () { setCur(full); } }, it.name)
          : h("a", { style: Object.assign({}, fnameBtn, { fontWeight: 600 }), onClick: function () { openItem(it); } }, it.name)),
      h("span", { style: { flex: "none", width: 90, textAlign: "right", opacity: .85 } }, dcsFmtSize(it.size)),
      h("span", { style: { flex: "none", width: 150, opacity: .6 } }, dcsFmtTime(it.mtime)),
      h("span", { style: { flex: "none", display: "flex", gap: 6 } }, actions)));
  });
  if (!busy && rows.length === 0) {
    listInner.push(h("div", { key: "empty", style: { padding: "14px 10px", opacity: .6 } }, "（空目录）"));
  }
  children.push(h("div", {
    ref: listRef,
    onScroll: function (e) { mem.scroll.list = e.currentTarget.scrollTop || 0; },
    style: { marginTop: 8, maxHeight: 260, overflowY: "auto", border: "1px solid rgba(128,128,128,.15)", borderRadius: 6 },
  }, busy ? h("div", { style: { padding: "14px 10px", opacity: .7 } }, "⏳ 加载中…") : h("div", {}, listInner)));
  if (err) {
    children.push(h("div", { style: { marginTop: 6, color: "#e5484d" } }, "⚠️ " + err));
  }
  if (prevBusy) {
    children.push(h("div", { style: { marginTop: 8, padding: 10, opacity: .7 } }, "⏳ 预览加载中…（文本/图片需先下载到本地缓存）"));
  }
  if (prev) {
    var inner = null;
    if (prev.type === "image") {
      inner = h("div", { style: { textAlign: "center", padding: 4 } }, h("img", { src: prev.content, style: { maxWidth: "100%", borderRadius: 6 } }));
    } else if (prev.type === "text") {
      inner = h("pre", { style: { margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all", fontSize: 11, fontFamily: "ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" } }, prev.content);
    } else {
      inner = h("div", { style: { padding: 4, opacity: .85 } }, prev.type === "pdf" ? "PDF 文件请点「下载」后用本地阅读器打开。" : ("该文件为二进制/过大（" + dcsFmtSize(prev.size) + "），不支持在线预览——点「下载」取回本机查看。"));
    }
    var headBar = h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 } },
      h("span", { style: { fontWeight: 700 } }, "👁 " + (prev.name || "") + " · " + dcsFmtSize(prev.size)),
      h("span", { style: { display: "flex", gap: 6 } },
        h("button", { style: btnStyle, onClick: function () { downloadFile({ name: prev.name }); } }, "⬇ 下载"),
        h("button", { style: btnStyle, onClick: closePreview }, "✕ 关闭")));
    var bodyBox = h("div", {
      ref: prevRef,
      onScroll: function (e) { mem.scroll.prev = e.currentTarget.scrollTop || 0; },
      style: { marginTop: 8, maxHeight: 420, overflowY: "auto" },
    }, inner);
    children.push(h("div", { style: { marginTop: 8, border: "1px solid rgba(15,98,254,.35)", borderRadius: 8, padding: "8px 10px" } }, headBar, bodyBox));
  }
...
function apply(ctx) {
  ...
  // 项目文件 tab（浏览当前项目容器文件：小文件在线预览 / 大文件下载）
  ctx.slots.inject("conversation.view", function () {
    return ctx.slots.register({ name: "conversation.view", id: "dcs-files", order: 62, label: "项目文件" }, guarded(FilesView));
  });
