...
function ModuleReviewPanel(props) {
  ...
  function act(action, text) {
    ...
    apiPost("/v2/module/review", {
      ...
      if (d && d.ok) {
        ...
        setMsg(({ approve: "✅ 已批准该节点", correct: "✏️ 已记录纠正意见，节点回到待审", veto: "⛔ 已否决该节点", propose: "💡 已提交新设想", comment: "💬 已提交意见" })[action] || "已提交");
      } else {
  ...
  return h("div", { className: "card", style: { marginTop: 10, borderLeft: "3px solid " + (review === 'approved' ? "#22a06b" : review === 'rejected' ? "#f5a524" : "#e5484d") } },
    ...
    // 审阅历史（人机对话留痕）
    log.length ? h("div", { style: { marginTop: 8, maxHeight: 140, overflow: "auto", fontSize: 12 } },
      log.slice().reverse().map(function (e, i) {
        var who = e.actor === 'ai' ? '🤖' : '👤';
        var actLabel = { propose: "设想", approve: "批准", correct: "纠正", veto: "否决", comment: "意见" }[e.action] || e.action;
        return h("div", { key: i, style: { padding: "3px 0", borderTop: "1px dashed rgba(128,128,128,.15)", opacity: .85 } },
          h("span", { style: { fontWeight: 600 } }, who + " " + actLabel + "："), " " + (e.text || "")
        );
      })
...
function layoutWorkflow(modules, plan, svgW) {
  ...
  for (var n = 0; n < order.length; n++) {
    ...
    nodes.push({
      ...
      isGate: !!order[n].isGate,
      review: order[n].review || 'none',
      desc: order[n].desc || "",
